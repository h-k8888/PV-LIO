...
void Preprocess::kitti_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    pcl::PointCloud<PointXYZIR>::Ptr pl_orig(new pcl::PointCloud<PointXYZIR>());
...
//        }
            ...
            PointType added_pt;
            const PointXYZIR& orig_pt = pl_orig->points[i];
            // cout<<"!!!!!!"<<i<<" "<<plsize<<endl;
